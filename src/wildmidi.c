...
static int write_dart_output (char *output_data, int output_size);
...
static int
write_dart_output (char *output_data, int output_size) {
	static int idx = 0;
...
static int write_sb_s16stereo(char *data, int siz) {
...
static int write_sb_u8stereo(char *data, int siz) {
...
static int write_sb_u8mono(char *data, int siz) {
...
int main(int argc, char **argv) {
	...
	char *output_buffer;
	unsigned long int perc_play;
	unsigned long int pro_mins;
	unsigned long int pro_secs;
	unsigned long int apr_mins;
	unsigned long int apr_secs;
	char modes[4];
		...
		while (optind < argc || test_midi) {
			...
			while (1) {
				...
				if (context != NULL) { /* OpenAL is active, not wav writer */
					unsigned short *swp = (unsigned short *) output_buffer;
					/* the library specifically outputs LE data
...
#endif
#if defined(AUDIODRV_AHI) && defined(WORDS_BIGENDIAN)
				if (AHIBuf[0] != NULL) { /* AHI is active, not wav writer */
					unsigned short *swp = (unsigned short *) output_buffer;
					/* the library specifically outputs LE data
					 * but AHI expects host-endian: do swap.  */
					for (i = 0; i < res / 2; ++i) {
						swp[i] = (swp[i] << 8) | (swp[i] >> 8);
					}
				}
