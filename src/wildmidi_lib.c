			...
			if (config_ptr != line_start_ptr) {
				...
				if (line_tokens) {
					if (wm_strcasecmp(line_tokens[0], "dir") == 0) {
						...
						if (!line_tokens[1] || !wm_isdigit(line_tokens[1][0])) {
							_WM_ERROR(__FUNCTION__, __LINE__, WM_ERR_INVALID_ARG,
									"(syntax error in reverb_room_width line)", 0);
							WM_FreePatches();
							free(config_dir);
							free(line_tokens);
							free(config_buffer);
						...
						if (!line_tokens[1] || !wm_isdigit(line_tokens[1][0])) {
							_WM_ERROR(__FUNCTION__, __LINE__, WM_ERR_INVALID_ARG,
									"(syntax error in reverb_room_length line)", 0);
							WM_FreePatches();
							free(config_dir);
							free(line_tokens);
							free(config_buffer);
						...
						if (!line_tokens[1] || !wm_isdigit(line_tokens[1][0])) {
							_WM_ERROR(__FUNCTION__, __LINE__, WM_ERR_INVALID_ARG,
									"(syntax error in reverb_listen_posx line)", 0);
							WM_FreePatches();
							free(config_dir);
							free(line_tokens);
							free(config_buffer);
						...
						if (!line_tokens[1] || !wm_isdigit(line_tokens[1][0])) {
							_WM_ERROR(__FUNCTION__, __LINE__, WM_ERR_INVALID_ARG,
									"(syntax error in reverb_listen_posy line)", 0);
							WM_FreePatches();
							free(config_dir);
							free(line_tokens);
							free(config_buffer);
...
WM_ParseNewMidi(unsigned char *midi_data, unsigned int midi_size) {
	...
	track_size = malloc(sizeof(unsigned int) * no_tracks);
	...
	while (end_of_tracks != no_tracks) {
		...
		for (i = 0; i < no_tracks; i++) {
			...
			do {
				...
				if (!track_size[i]) goto shortbuf;
				current_event_ch = current_event & 0x0F;
				switch (current_event >> 4) {
				case 0x8:
					NOTEOFF:
					if (track_size[i] < 2) goto shortbuf;
					midi_setup_noteoff(mdi, current_event_ch,
				...
				case 0xB:
					...
					break;
				case 0xC:
					if (track_size[i] < 1) goto shortbuf;
				...
				case 0xF: /* Meta Event */
					if (current_event == 0xFF) {
						...
						if (tracks[i][0] == 0x02) { /* Copyright Event */
							...
							tmp_length = (tmp_length << 7)
									+ (*tracks[i] & 0x7f);
							if (--track_size[i] < tmp_length) goto shortbuf;
							...
							goto NEXT_TRACK;
								...
								&& (tracks[i][1] == 0x03)) {
							/* Tempo */
							if (track_size[i] < 5) goto shortbuf;
						...
						sysex_len = (sysex_len << 7) + (*tracks[i] & 0x7F);
						tracks[i]++;
						if (--track_size[i] < sysex_len) goto shortbuf;
