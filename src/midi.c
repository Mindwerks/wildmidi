...
#if 0
    /*
        This will be done using a seperate WM_ParseNewXmi/Mus
...
WM_Event2Midi(struct _mdi *mdi, uint8_t **out, uint32_t *outsize) {
    ...
    for (i = 0; i < mdi->event_count; i++) {
        ...
        if (mdi->events[i].do_event == NULL) {
            ...
            (*out)[out_ofs++] = 64;
            (*out)[out_ofs++] = mdi->events[i].event_data.data & 0xff;
        } else if (mdi->events[i].do_event == do_control_data_increment) {
            // DEBUG
            ...
            (*out)[out_ofs++] = 96;
            (*out)[out_ofs++] = mdi->events[i].event_data.data & 0xff;
        } else if (mdi->events[i].do_event == do_control_data_decrement) {
            // DEBUG
            ...
            (*out)[out_ofs++] = 97;
            (*out)[out_ofs++] = mdi->events[i].event_data.data & 0xff;
        } else if (mdi->events[i].do_event == do_control_non_registered_param_fine) {
            // DEBUG
        ...
        } else if (mdi->events[i].do_event == do_control_non_registered_param_course) {
            // DEBUG
            // fprintf(stderr,"Control Non Registered Param: %u %.4x\r\n",mdi->events[i].event_data.channel, mdi->events[i].event_data.data);
            if (running_event != (0xb0 | mdi->events[i].event_data.channel)) {
                (*out)[out_ofs++] = 0xb0 | mdi->events[i].event_data.channel;
            ...
            out_ofs += 9;
            running_event = 0;
        } else if (mdi->events[i].do_event == do_meta_endoftrack) {
            // DEBUG
            // fprintf(stderr,"End Of Track\r\n");
        } else if (mdi->events[i].do_event == do_meta_tempo) {
            // DEBUG
